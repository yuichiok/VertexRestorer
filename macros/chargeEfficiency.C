...
void drawLegend()
{
	...
	eff1->SetLineColor(kGray+2);
	eff1->SetLineWidth(2);
	eff1->SetMarkerSize(1);
	eff1->SetMarkerStyle(20);
	eff1->SetMarkerColor(kGray+2);
	...
	TLegend *legendMean2 = new TLegend(0.65,0.7,0.9,0.92,NULL,"brNDC");
	legendMean2->SetHeader("IDR-L");
        legendMean2->SetFillColor(kWhite);
        legendMean2->SetBorderSize(0);
        legendMean2->AddEntry(eff21,"after recovery","pel");
	legendMean2->AddEntry(eff1,"before recovery","pel");
	legendMean2->Draw();
}
void chargeEfficiency(string recofilename = "250GeV_eL_before/OUTPUT3_bbbar_eL.root", int color = kBlue, TCanvas * c1 = NULL, string genfilename = "/home/irles/WorkArea/BBbar_tests/ntuples/trackrestoring/IDR_vertex_reprocessed/OUTPUT2_eL_l5_merged.root", TString topology="")
//void chargeEfficiency()
{
  std::string title;
  if(topology=="bbbar")  title="e_{L}^{-}e_{R}^{+}#rightarrowb#bar{b} @500 GeV";
  else if (topology=="ttbar")  title="e_{L}^{-}e_{R}^{+}#rightarrow t#bar{t} (l#nu+jets) @500 GeV";
	...
	if (!c1) 
	{
		c1 = new TCanvas("c1", "Data-MC",0,0,2100,1700);
		c1->Divide(2,2);
		same = false;
	}
	...
	std::string title_1 = title + ";btag;Charge purity";
	getEfficiency(btagreco, btagrecototal, same, color, title_1);
	...
	gPad->SetLeftMargin(0.15);
	gPad->SetRightMargin(0.03);
	std::string title_2 = title + ";|p|_{had};Charge purity";
	getEfficiency(momentumreco, momentumrecototal, same, color, title_2);
	...
	//gPad->SetLeftMargin(0.15);
	//	gPad->SetRightMargin(0.03);
	std::string title_3 = title + ";N_{rec};Charge purity";
	getEfficiency(numberreco, numberrecototal, same, color, title_3);
	...
	std::string title_4 = title + ";|cos#theta|;Charge purity";
	getEfficiency(cosgen, cosgentotal, same, color, title_4 );
